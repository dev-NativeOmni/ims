...
return new class extends Migration
{
    ...
    public function up(): void
    {
        $juzRanges = [
            ...
            15 => [['surah' => 17, 'start' => 1, 'end' => 111], ['surah' => 18, 'start' => 1, 'end' => 74]], // Note: some lists group 74 in Juz 15, some 18:75 in 16. We align with the boundary.
        ...
        DB::transaction(function () use ($juzRanges) {
            foreach ($juzRanges as $juz => $ranges) {
                foreach ($ranges as $range) {
                    // Update all ayahs in this surah and ayah number range to belong to this juz
                    DB::table('ayahs')
                        ->where('surah_id', $range['surah'])
                        ->whereBetween('ayah_number', [$range['start'], $range['end']])
                        ->update(['juz' => $juz]);
                }
            }
        });
