<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        @if (! data_get($stats, 'teacher'))
            <div class="p-4 rounded-2xl bg-rose-500/15 border border-rose-500/30 text-rose-800 dark:text-rose-300 text-sm font-bold backdrop-blur-md">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="glass-card glass-card-hover rounded-3xl p-5 flex flex-col justify-between">
                ...
                <div class="mt-4">
                    <span class="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider block">Murid Bimbingan</span>
            ...
            <div class="glass-card glass-card-hover rounded-3xl p-5 flex flex-col justify-between">
                <div class="flex items-center justify-between">
                    <div class="w-10 h-10 rounded-2xl bg-rose-500/15 text-rose-700 dark:text-rose-300 flex items-center justify-center border border-white/60 shadow-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                ...
                <div class="mt-4">
                    <span class="text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider block">Butuh Perhatian</span>
                    <p class="text-3xl font-black text-zinc-900 dark:text-white tracking-tight mt-0.5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <a href="{{ url('/hafalan-records/create') }}" class="glass-card glass-card-hover rounded-3xl p-5 flex items-center gap-4 group cursor-pointer">
                ...
                <div>
                    <h4 class="font-extrabold text-sm text-zinc-900 dark:text-white group-hover:text-emerald-700 dark:group-hover:text-emerald-300 transition-colors">Input Setoran</h4>
                    <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-0.5">Catat setoran hafalan baru murid.</p>
                </div>
            </a>
            ...
            <a href="{{ url('/murajaah-records/create') }}" class="glass-card glass-card-hover rounded-3xl p-5 flex items-center gap-4 group cursor-pointer">
                ...
                <div>
                    <h4 class="font-extrabold text-sm text-zinc-900 dark:text-white group-hover:text-amber-700 dark:group-hover:text-amber-300 transition-colors">Input Muraja'ah</h4>
                    <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-0.5">Catat pengulangan hafalan.</p>
                </div>
            </a>
            ...
            <a href="{{ url('/hafalan-targets/create') }}" class="glass-card glass-card-hover rounded-3xl p-5 flex items-center gap-4 group cursor-pointer">
                ...
                <div>
                    <h4 class="font-extrabold text-sm text-zinc-900 dark:text-white group-hover:text-teal-700 dark:group-hover:text-teal-300 transition-colors">Buat Target</h4>
                    <p class="text-xs text-zinc-600 dark:text-zinc-400 mt-0.5">Tetapkan target hafalan murid.</p>
                </div>
            </a>
        </div>
        ...
        <div class="glass-card rounded-3xl overflow-hidden p-5 sm:p-6">
            <div class="flex items-center justify-between pb-4 border-b border-white/40 dark:border-white/10">
                <div>
                    <h3 class="font-extrabold text-base text-zinc-900 dark:text-white tracking-tight">Progress Murid Bimbingan</h3>
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Capaian santri dalam kelompok bimbingan Anda.</p>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-white/20 dark:divide-white/5">
                        @forelse ($studentsProgress as $item)
                            ...
                            <tr class="hover:bg-white/30 dark:hover:bg-white/5 transition-colors">
                                <td class="py-3.5 px-3 font-bold text-zinc-900 dark:text-white">{{ $student?->name ?? data_get($item, 'student_name', '-') }}</td>
                                <td class="py-3.5 px-3 text-zinc-600 dark:text-zinc-400 font-medium">{{ $student?->classRoom?->name ?? data_get($item, 'class_room_name', '-') }}</td>
                                ...
                                <td class="py-3.5 px-3 font-bold text-zinc-700 dark:text-zinc-300">{{ $activeTargetCount }}</td>
                                <td class="py-3.5 px-3 font-bold {{ $overdueTargetCount > 0 ? 'text-rose-600' : 'text-zinc-500' }}">{{ $overdueTargetCount }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="py-8 text-center text-xs text-zinc-500">Belum ada murid bimbingan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="glass-card rounded-3xl p-5 sm:p-6 space-y-4">
                <div class="flex items-center justify-between pb-3 border-b border-white/40 dark:border-white/10">
                    ...
                    <a href="{{ url('/hafalan-targets') }}" class="text-xs font-bold text-teal-800 dark:text-teal-300 hover:underline">Kelola &rarr;</a>
                </div>
                        ...
                        <div class="pt-3 first:pt-0 flex justify-between gap-4">
                            <div>
                                ...
                                @if ($target->ummi_jilid)
                                    <p class="text-xs font-bold text-teal-800 dark:text-teal-300 mt-0.5">📗 {{ $target->ummi_jilid }} (Peraga: {{ $target->halaman_peraga ?? '-' }}, Buku: {{ $target->halaman_buku ?? '-' }})</p>
                                @else
                                    <p class="text-xs font-medium text-zinc-600 dark:text-zinc-400 mt-0.5">{{ $target->surah?->name_latin ?? '-' }} ayat {{ $target->ayah_range }}</p>
                                @endif
                            </div>
                            <div class="text-right">
                                <p class="text-xs font-bold text-zinc-900 dark:text-white">{{ $target->target_date?->format('d M Y') }}</p>
            ...
            <div class="glass-card rounded-3xl p-5 sm:p-6 space-y-4">
                ...
                <div class="divide-y divide-white/20 dark:divide-white/5 space-y-2">
                    @forelse ($latestHafalanRecords as $record)
                        <div class="pt-3 first:pt-0 flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p class="text-xs font-medium text-zinc-600 dark:text-zinc-400 mt-0.5">{{ $record->surah?->name_latin ?? '-' }} ayat {{ $record->ayah_range }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs font-bold text-zinc-700 dark:text-zinc-300">{{ $record->submitted_at?->format('d M Y') }}</p>
                    ...
                    @empty
                        <div class="py-6 text-center text-xs text-zinc-500">Belum ada setoran.</div>
                    @endforelse
                </div>
            </div>
        </div>
