    ...
    <div>
        ...
        <div x-data="{ open: false, search: '' }" @click.outside="open = false" class="relative mt-1">
            <button type="button" 
                ...
                <span x-text="selectedStudent && allStudents.find(s => s.id == selectedStudent) ? allStudents.find(s => s.id == selectedStudent).name + (allStudents.find(s => s.id == selectedStudent).className ? ' — ' + allStudents.find(s => s.id == selectedStudent).className : '') : 'Pilih Santri'"></span>
