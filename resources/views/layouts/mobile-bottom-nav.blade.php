...
<nav class="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#09090b]/95 backdrop-blur-xl border-t border-zinc-200/80 dark:border-white/10 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] dark:shadow-[0_-4px_20px_rgba(0,0,0,0.5)] transition-all duration-200 pb-[max(env(safe-area-inset-bottom),8px)]" aria-label="Navigasi Bawah">
    <div class="flex items-center justify-around h-14 sm:h-16 px-1 max-w-lg mx-auto">
        @foreach ($navItems as $item)
            @if (\Illuminate\Support\Facades\Route::has($item['route']))
                <a href="{{ route($item['route']) }}"
                   class="relative flex-1 flex flex-col items-center justify-center h-full py-1 text-center group select-none transition-all duration-150 {{ $item['active'] ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-zinc-400 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 font-medium' }}">
                    ...
                    <span class="text-[9.5px] sm:text-[10px] mt-0.5 tracking-tight truncate max-w-full px-0.5 leading-none">{{ $item['label'] }}</span>
                </a>
            @endif
        @endforeach
        ...
        <button type="button"
                ...
                class="relative flex-1 flex flex-col items-center justify-center h-full py-1 text-center group select-none text-zinc-400 dark:text-zinc-400 hover:text-indigo-600 dark:hover:text-indigo-400 font-medium transition-all duration-150 cursor-pointer">
            <div class="p-1 rounded-xl transition-all duration-200 group-active:scale-95 group-hover:bg-zinc-100 dark:group-hover:bg-white/5">
                <svg class="w-5 h-5 shrink-0 text-zinc-400 dark:text-zinc-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
                </svg>
            </div>
            <span class="text-[9.5px] sm:text-[10px] mt-0.5 tracking-tight leading-none">Lainnya</span>
        </button>
    </div>
</nav>
