@php
    ...
    $getLinkClasses = function (bool $active): string {
        return $active
            ? 'flex items-center px-3.5 py-2.5 text-sm font-bold rounded-2xl bg-white dark:bg-white/20 text-slate-950 dark:text-white shadow-[0_4px_16px_rgba(0,0,0,0.08)] border border-white/80 dark:border-white/20 group transition-all duration-200'
            : 'flex items-center px-3.5 py-2.5 text-sm font-semibold rounded-2xl text-slate-800 dark:text-zinc-200 hover:bg-white/70 dark:hover:bg-white/10 hover:text-slate-950 dark:hover:text-white group transition-all duration-150';
    };

    $getIconClasses = function (bool $active): string {
        return $active
            ? 'mr-3 h-5 w-5 text-teal-700 dark:text-teal-300 flex-shrink-0 transition-colors duration-150'
            : 'mr-3 h-5 w-5 text-slate-600 dark:text-zinc-400 group-hover:text-slate-950 dark:group-hover:text-white flex-shrink-0 transition-colors duration-150';
    };
@endphp
...
<div x-show="sidebarOpen" class="fixed inset-0 flex z-50" role="dialog" aria-modal="true" style="display: none;">
    ...
    <div x-show="sidebarOpen" 
         ...
         class="fixed inset-0 bg-zinc-950/70 backdrop-blur-md" aria-hidden="true"></div>

    <!-- Floating Sidebar Drawer Panel (High Contrast Glass) -->
    <div x-show="sidebarOpen"
         ...
         class="relative flex-1 flex flex-col max-w-xs w-full m-3 sm:m-4 rounded-3xl glass-sidebar p-4 shadow-2xl transition-all duration-200 border border-white/70 dark:border-white/10 z-50">
         
         <!-- Close Button -->
         <div class="absolute top-3 right-3">
             <button type="button" @click="sidebarOpen = false" class="glass-circle-btn w-8 h-8 focus:outline-none" title="Tutup">
                 <span class="sr-only">Tutup sidebar</span>
                 <svg class="h-4 w-4 text-slate-800 dark:text-zinc-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
         ...
         <div class="flex-shrink-0 flex items-center gap-3 px-2 pt-2 pb-4 border-b border-white/40 dark:border-white/10">
              ...
              @else
                  <div class="h-9 w-9 rounded-2xl bg-teal-500/20 text-teal-800 dark:text-teal-300 flex items-center justify-center flex-shrink-0 shadow-sm border border-white/60">
              ...
              <div class="min-w-0 pr-8">
                  <h1 class="font-extrabold text-sm sm:text-base text-slate-950 dark:text-white leading-tight tracking-tight">
         ...
         <div class="flex-shrink-0 mt-3 pt-3 border-t border-white/40 dark:border-white/10">
             <div class="p-3 rounded-2xl bg-white/70 dark:bg-white/10 border border-white/80 dark:border-white/10 shadow-sm flex items-center gap-3">
                 ...
                 <div class="min-w-0 flex-1">
                     <p class="text-xs font-bold text-slate-950 dark:text-white truncate leading-tight">
                         {{ $user?->name }}
                     </p>
                     <p class="text-[10px] font-semibold text-slate-600 dark:text-zinc-300 truncate leading-tight mt-0.5">
             ...
             <div class="flex items-center gap-1.5 mt-2">
                 @if ($hasRoute('profile.edit'))
                     <a href="{{ route('profile.edit') }}" class="flex-1 inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl text-[11px] font-bold text-slate-800 dark:text-zinc-200 hover:text-slate-950 dark:hover:text-white bg-white/60 dark:bg-white/10 hover:bg-white/90 dark:hover:bg-white/20 border border-white/70 dark:border-white/10 transition-all duration-150">
                 ...
                 <form method="POST" action="{{ route('logout') }}" class="flex-1">
                     @csrf
                     <button type="submit" class="w-full inline-flex items-center justify-center gap-1 px-2.5 py-1.5 rounded-xl text-[11px] font-bold text-rose-700 dark:text-rose-400 bg-rose-50/70 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/50 border border-rose-200/70 dark:border-rose-800/50 transition-all duration-150 cursor-pointer">
                         <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<!-- Global Top Bar (Full-Width Sticky Header with Clean Boundary) -->
<header class="sticky top-0 z-30 w-full glass-header-bar transition-all duration-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3 min-w-0">
            <button type="button" @click="sidebarOpen = true" class="glass-circle-btn flex-shrink-0" title="Menu Navigasi">
                ...
                <svg class="h-5 w-5 text-slate-800 dark:text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex items-center gap-2.5 min-w-0">
                ...
                @endif
                <span class="font-extrabold text-sm sm:text-base md:text-lg text-slate-900 dark:text-white tracking-tight leading-tight">
                    {{ $namaInstansi ?: 'IMS Tahfizh' }}
                </span>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2.5 sm:gap-3 flex-shrink-0">
            ...
            <button type="button" @click="toggleTheme()" class="glass-circle-btn" title="Ubah Tema">
                <svg x-show="dark" class="w-4 h-4 text-amber-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" style="display: none;">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m0-12.728l.707.707m12.728 12.728l.707-.707M12 8a4 4 0 100 8 4 4 0 000-8z" />
                </svg>
                <svg x-show="!dark" class="w-4 h-4 text-slate-800" fill="none" viewBox="0 0 24 24" stroke="currentColor">
