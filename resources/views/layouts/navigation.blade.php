...
<div x-show="sidebarOpen" class="fixed inset-0 flex z-40" role="dialog" aria-modal="true" style="display: none;">
    ...
    <div x-show="sidebarOpen"
         ...
         <div class="flex-shrink-0 flex items-center px-4">
             <span class="font-bold text-lg text-zinc-900 dark:text-white tracking-tight flex items-center gap-3">
                  @if ($logoUrl)
                  ...
                  @endif
                 <span class="font-bold text-sm sm:text-base text-zinc-900 dark:text-white leading-snug">{{ $namaInstansi ?: 'IMS' }}</span>
             </span>
         </div>
...
<div class="sticky top-0 z-30 flex h-16 bg-white/85 dark:bg-[#09090b]/60 backdrop-blur-xl border-b border-zinc-200 dark:border-white/5 flex-shrink-0 transition-colors duration-200">
    ...
    <div class="flex-1 flex justify-between px-4 items-center min-w-0">
        <!-- Left: Brand Logo & Full Name -->
        <div class="flex items-center gap-3 min-w-0 mr-3">
            <span class="font-bold text-zinc-800 dark:text-white tracking-tight flex items-center gap-2.5 min-w-0">
                @if ($logoUrl)
                ...
                @endif
                <span class="font-bold text-sm sm:text-base md:text-lg text-zinc-800 dark:text-white tracking-tight leading-tight">{{ $namaInstansi ?: 'IMS' }}</span>
            </span>
        </div>
        ...
        <div class="flex items-center gap-2.5 sm:gap-3 flex-shrink-0">
            <!-- Theme Toggle -->
            <button type="button" @click="toggleTheme()" class="p-2 rounded-xl bg-zinc-100 dark:bg-white/5 border border-zinc-200 dark:border-white/10 text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-200/60 dark:hover:bg-white/10 transition-all duration-150 flex items-center justify-center cursor-pointer shadow-sm" title="Ubah Tema">
            ...
            @if ($user?->avatar)
                <button type="button" @click="sidebarOpen = true" class="w-8 h-8 rounded-full overflow-hidden border border-zinc-200 dark:border-white/10 shadow-inner hover:ring-2 hover:ring-indigo-500/40 transition cursor-pointer">
                    <img src="{{ asset('storage/' . $user->avatar) }}" alt="{{ $user->name }}" class="w-full h-full object-cover">
                </button>
            @else
                <button type="button" @click="sidebarOpen = true" class="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20 flex items-center justify-center font-bold text-xs uppercase shadow-inner hover:ring-2 hover:ring-indigo-500/40 transition cursor-pointer">
                    {{ substr($user?->name ?? 'U', 0, 1) }}
                </button>
            @endif
        </div>
