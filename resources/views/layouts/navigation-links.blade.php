...
@if ($canManageRecords || ($canViewProgress && $hasRoute('progress.index')) || ($canViewReports && $hasRoute('reports.index')))
    <div class="mt-6 space-y-1">
        ...
        @if ($canViewReports && $hasRoute('reports.index'))
            <a href="{{ route('reports.index') }}" class="{{ $getLinkClasses($routeIs('reports.*')) }}">
                <svg class="{{ $getIconClasses($routeIs('reports.*')) }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
