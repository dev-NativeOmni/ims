@php
    ...
    $canViewReportSettings = $isSuperAdmin || $isAdminUser;
...
@if (($canViewTahfizhGroup || $canViewDigitalReports) && ($canManageRecords || ($canViewProgress && $hasRoute('progress.index')) || ($canViewReports && $hasRoute('reports.index')) || ($canViewPeriodicReports && $hasRoute('reports.periodic')) || ($canViewMushaf && $hasRoute('quran.mushaf')) || ($canViewDigitalReports && $hasRoute('digital-reports.index')) || ($canViewTeacherPerformance && $hasRoute('reports.teachers'))))
    <div class="mt-6 space-y-1">
        ...
        @if ($canManageRecords)
            ...
            @if (($isSuperAdmin || $isAdminUser) && $hasRoute('settings.tahfizh-scoring'))
                <a href="{{ route('settings.tahfizh-scoring') }}" class="{{ $getLinkClasses($routeIs('settings.tahfizh-scoring*')) }}">
                    <svg class="{{ $getIconClasses($routeIs('settings.tahfizh-scoring*')) }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if ($canViewReportSettings && $hasRoute('digital-reports.settings'))
