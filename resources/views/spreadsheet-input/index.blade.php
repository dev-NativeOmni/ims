        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('spreadsheetData', () => ({
                ...
                barisHint(h, estimate) {
                    ...
                    if (estimate === null) return manual ? 'Baris manual' : 'Perkiraan: -';
                    if (!manual) return 'Perkiraan: ' + estimate + ' baris';
                                                            ...
                                                            <template x-for="(h, hIndex) in cell.hafalans" :key="hIndex">
                                                                <div class="p-2 bg-gray-50/50 dark:bg-zinc-800/40 border border-gray-255 dark:border-zinc-800 rounded-lg relative space-y-1.5">
                                                                    ...
                                                                    <div class="flex items-center gap-1.5">
                                                                        <input type="text" inputmode="decimal" :name="isMobileView ? '' : 'records[' + student.id + '][dates][' + date + '][hafalans][' + hIndex + '][baris]'" x-model="h.baris" @input="isDirty = true" :placeholder="hafalanEstimate(h) !== null ? '≈ ' + hafalanEstimate(h) : 'Baris'" :disabled="isMobileView || tab !== 'hafalan' || (cell.attendance && cell.attendance !== 'hadir')" title="Baris manual (kosongkan untuk memakai perkiraan)" class="block w-16 shrink-0 rounded border-gray-300 dark:border-zinc-700 bg-transparent text-[11px] px-2 py-0.5 dark:text-white">
                                                        ...
                                                        <div x-show="tab === 'ummi'" class="space-y-2">
                                                                    ...
                                                                    <div class="border-t border-gray-150 dark:border-zinc-800 pt-2 space-y-1.5">
                                                                        ...
                                                                        <template x-for="(h, hIndex) in cell.ummiHafalans" :key="hIndex">
                                                                            <div class="p-1.5 bg-gray-50/50 dark:bg-zinc-850 border border-gray-200 dark:border-zinc-800 rounded relative space-y-1">
                                                                                ...
                                                                                <div class="flex items-center gap-1.5">
                                                                        <input type="text" inputmode="decimal" :name="'records[' + student.id + '][dates][' + date + '][hafalans][' + hIndex + '][baris]'" x-model="h.baris" @input="isDirty = true" :placeholder="ummiEstimate(h) !== null ? '≈ ' + ummiEstimate(h) : 'Baris'" :disabled="tab !== 'ummi' || cell.attendance !== 'hadir'" title="Baris manual (kosongkan untuk memakai perkiraan)" class="block w-16 shrink-0 rounded border-gray-300 dark:border-zinc-700 bg-transparent text-[10px] px-2 py-0.5 dark:text-white">
                                                            ...
                                                            <template x-if="student.tahfizh_level !== 'ummi'">
                                                                <div class="space-y-2">
                                                                    ...
                                                                    <div class="flex items-center gap-1.5">
                                                                        <input type="text" inputmode="decimal" :name="'records[' + student.id + '][dates][' + date + '][hafalans][' + hIndex + '][baris]'" x-model="h.baris" @input="isDirty = true" :placeholder="hafalanEstimate(h) !== null ? '≈ ' + hafalanEstimate(h) : 'Baris'" :disabled="tab !== 'hafalan' || cell.attendance !== 'hadir'" title="Baris manual (kosongkan untuk memakai perkiraan)" class="block w-16 shrink-0 rounded border-gray-300 dark:border-zinc-700 bg-transparent text-[11px] px-2 py-0.5 dark:text-white">
                                                                        <span class="text-[11px] leading-tight" :class="(h.baris !== undefined && h.baris !== null && String(h.baris).trim() !== '' && hafalanEstimate(h) !== null && Math.abs(parseFloat(String(h.baris).replace(',', '.')) - hafalanEstimate(h)) >= 0.01) ? 'text-amber-600 font-semibold' : 'text-gray-400'" x-text="barisHint(h, hafalanEstimate(h))"></span>
                                                                    </div>
                                                                            <!-- Score & Status -->
                                                                            <div class="grid grid-cols-2 gap-1">
                                                    ...
                                                    <div class="grid grid-cols-2 gap-2">
                                                        ...
                                                        <div>
                                                            ...
                                                            <input type="number" :name="!isMobileView ? '' : 'records[' + student.id + '][dates][' + selectedMobileDate + '][hafalans][' + hIndex + '][ayah_end]'" x-model.number="h.ayah_end" @input="autoMarkHadir(student.id, selectedMobileDate)" placeholder="Akhir" :disabled="!isMobileView || tab !== 'hafalan' || (cell.attendance && cell.attendance !== 'hadir')" class="block w-full rounded border-gray-300 dark:border-zinc-700 bg-transparent text-xs py-1 dark:text-white">
                                                                    ...
                                                                    <div class="flex items-center gap-1.5">
                                                                        <input type="text" inputmode="decimal" :name="!isMobileView ? '' : 'records[' + student.id + '][dates][' + selectedMobileDate + '][hafalans][' + hIndex + '][baris]'" x-model="h.baris" @input="isDirty = true" :placeholder="hafalanEstimate(h) !== null ? '≈ ' + hafalanEstimate(h) : 'Baris'" :disabled="!isMobileView || tab !== 'hafalan' || (cell.attendance && cell.attendance !== 'hadir')" title="Baris manual (kosongkan untuk memakai perkiraan)" class="block w-16 shrink-0 rounded border-gray-300 dark:border-zinc-700 bg-transparent text-xs px-2 py-0.5 dark:text-white">
                                                        ...
                                                        <div class="bg-gray-50/50 dark:bg-zinc-800/40 p-3 rounded-lg border border-gray-255 dark:border-zinc-800 relative space-y-3">
                                                            ...
                                                            <div class="grid grid-cols-2 gap-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div class="flex items-center gap-1.5">
                                                                        <input type="text" inputmode="decimal" :name="'records[' + student.id + '][dates][' + selectedMobileDate + '][hafalans][' + hIndex + '][baris]'" x-model="h.baris" @input="isDirty = true" :placeholder="hafalanEstimate(h) !== null ? '≈ ' + hafalanEstimate(h) : 'Baris'" :disabled="tab !== 'hafalan' || cell.attendance !== 'hadir'" title="Baris manual (kosongkan untuk memakai perkiraan)" class="block w-16 shrink-0 rounded border-gray-300 dark:border-zinc-700 bg-transparent text-xs px-2 py-0.5 dark:text-white">
                                                                        <span class="text-xs leading-tight" :class="(h.baris !== undefined && h.baris !== null && String(h.baris).trim() !== '' && hafalanEstimate(h) !== null && Math.abs(parseFloat(String(h.baris).replace(',', '.')) - hafalanEstimate(h)) >= 0.01) ? 'text-amber-600 font-semibold' : 'text-gray-400'" x-text="barisHint(h, hafalanEstimate(h))"></span>
                                                                    </div>
                                                            </div>
                                                            <div class="grid grid-cols-2 gap-2">
