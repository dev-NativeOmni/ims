                                ...
                                <template x-for="student in students" :key="student.id">
                                    <tr class="hover:bg-gray-50/30 dark:hover:bg-zinc-850/10">
                                        ...
                                        <td class="sticky left-0 z-10 bg-white dark:bg-zinc-900 px-4 py-3 border-r dark:border-zinc-800 font-bold text-xs text-gray-900 dark:text-zinc-200 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                                            ...
                                            <span class="block text-[10px] text-gray-400 font-medium mt-0.5" x-text="student.level === 'ummi' ? 'Level: UMMI' : 'Level: ' + student.level"></span>
                                        </td>
                                            ...
                                            <td class="p-3 border-r dark:border-zinc-800 align-top">
                                                <div class="space-y-2" x-data="{ cell: gridData[student.id].dates[date] }">
                                                    ...
                                                    <div :class="cell.attendance !== 'hadir' ? 'opacity-30 pointer-events-none' : ''" class="transition-opacity space-y-2">
                                                        ...
                                                        <!-- TAB 1: SETORAN AL-QUR'AN -->
                                                        <div x-show="tab === 'hafalan'" class="space-y-2">
                                                            <template x-for="(h, hIndex) in cell.hafalans" :key="hIndex">
                                                                <div class="p-2 bg-gray-50/50 dark:bg-zinc-800/40 border border-gray-250 dark:border-zinc-800 rounded-lg relative space-y-1.5">
                                                                    <!-- Surah select -->
                            ...
                            <template x-for="student in students" :key="student.id">
                                <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl p-4 shadow-sm space-y-4" x-data="{ cell: gridData[student.id].dates[selectedMobileDate] }">
                                    ...
                                    <div class="flex items-center justify-between border-b dark:border-zinc-800 pb-2.5">
                                        ...
                                        <span class="px-2 py-0.5 rounded bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-zinc-400 font-bold text-[10px]" x-text="student.level === 'ummi' ? 'Level: UMMI' : 'Level: ' + student.level"></span>
                                    </div>
                                    ...
                                    <div :class="cell.attendance !== 'hadir' ? 'opacity-30 pointer-events-none' : ''" class="transition-opacity space-y-4">
                                        ...
                                        <div x-show="tab === 'hafalan'" class="space-y-3">
                                            <template x-for="(h, hIndex) in cell.hafalans" :key="hIndex">
                                                <div class="bg-gray-50/50 dark:bg-zinc-800/40 p-3 rounded-lg border border-gray-250 dark:border-zinc-800 relative space-y-3">
                                                    <div>
                                                    ...
                                                    <!-- Delete button -->
                                                    <template x-if="cell.hafalans.length > 1">
                                                        <button type="button" @click="cell.hafalans.splice(hIndex, 1)" class="absolute top-2 right-2 text-rose-650 text-xs font-bold bg-white dark:bg-zinc-800 border border-gray-250 dark:border-zinc-700 rounded-full w-5 h-5 flex items-center justify-center cursor-pointer shadow-sm">×</button>
                                                    </template>
                                                </div>
                                            </template>
                                            ...
                                            <div class="border-t border-gray-150 dark:border-zinc-800 pt-3.5 space-y-2">
                                                ...
                                                <template x-for="(h, hIndex) in cell.ummiHafalans" :key="hIndex">
                                                    <div class="bg-gray-50/50 dark:bg-zinc-855 p-3 rounded-lg border border-gray-200 dark:border-zinc-800 relative space-y-2">
                                                        ...
                                                        <!-- Remove button -->
                                                        <template x-if="cell.ummiHafalans.length > 1">
                                                            <button type="button" @click="cell.ummiHafalans.splice(hIndex, 1)" class="absolute top-2 right-2 text-rose-650 text-xs font-bold bg-white dark:bg-zinc-800 border border-gray-250 dark:border-zinc-700 rounded-full w-5 h-5 flex items-center justify-center cursor-pointer shadow-sm">×</button>
                                                        </template>
                                                    </div>
                                                </template>
                                                <button type="button" @click="cell.ummiHafalans.push({ id: null, surah_id: '', ayah: '' })" :disabled="cell.attendance !== 'hadir'" class="w-full py-1.5 bg-emerald-50 hover:bg-emerald-100 dark:bg-zinc-800 text-emerald-650 dark:text-emerald-450 border border-emerald-200 dark:border-zinc-700 rounded-lg text-xs font-bold transition cursor-pointer">
                                                    + Tambah Hafalan
                                                </button>
                                            </div>
                                        </div>
